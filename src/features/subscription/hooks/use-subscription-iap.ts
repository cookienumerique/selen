...
export type SubscriptionBasePlanId = 'selen-premium-monthly' | 'selen-premium-yearly';

export const useSubscriptionIap = () => {
    const listenersInitialized = useRef<boolean>(false);

    useIapInit();
    const [subscriptionsIap, setSubscriptionsIap] = useState<ProductOrSubscription[]>([]);
    const { mutateAsync: subscribeApi, isPending: isLoadingSubscribeApi } = useSubscribeApi();
        ...
        updateListener = purchaseUpdatedListener(
            async (purchaseData: Purchase) => {
                ...
                try {
                    ...
                    const { item: subscriptionCreated } = await subscribeApi({ purchaseToken, productId });
    ...
    const buy = async (basePlanId: SubscriptionBasePlanId) => {
        try {
            ...
            const product = subscriptionsIap.find(s => s.id === subscriptionProductId);
