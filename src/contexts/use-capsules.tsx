import { CreateCapsuleResponsePayload, useCreateCapsuleResponse } from "@/src/features/capsule-reponse/hooks/use-create";
...
type CapsulesContextReturn = {
  ...
  createCapsuleResponse: ({ capsuleId, response }: CreateCapsuleResponsePayload) => void;
...
export function CapsulesProvider({ children }: { children: ReactNode }) {
  const { data: capsules = [], isLoading: isLoadingCapsules } = useFetchCapsules();
  const { data: capsulesResponses = [], isLoading: isLoadingCapsulesResponses, refetch: refetchCapsulesResponses } = useFetchCapsulesResponse();
  const { mutateAsync: createCapsuleResponseMutation, isPending: isLoadingCreateCapsuleResponseMutation } = useCreateCapsuleResponse();
  const capsuleAlreadyRespondedToday: boolean = capsulesResponses.some((capsule) =>
    dayjs(capsule.createdAt).isSame(dayjs(), "day")
  );

  const createCapsuleResponse = async (payload: CreateCapsuleResponsePayload) => {
